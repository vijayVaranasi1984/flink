...
public class EmpCount {
	...
	public static final MapStateDescriptor<String, String> excludeEmpDescriptor = new MapStateDescriptor<String, String>(
			"exclude_employ", BasicTypeInfo.STRING_TYPE_INFO, BasicTypeInfo.STRING_TYPE_INFO);
	...
	public static void main(String[] args) throws Exception {
		...
		DataStream<Tuple2<String, Integer>> employees = env.readTextFile("/Flink/state/broadcast/broadcast.txt")
				.map(new MapFunction<String, Tuple2<String, String>>() {
					public Tuple2<String, String> map(String value) {
						// dept, data
						return new Tuple2<String, String>(value.split(",")[3], value); // {(Purchase),
																						// (AXPM175755,Nana,Developer,Purchase,GH67D)}
					}
				}).keyBy(0).connect(excludeEmpBroadcast) // will return a BroadcastConnectedStream
				.process(new ExcludeEmp());

		employees.writeAsText("/Flink/state/broadcast/bd_out");
	...
	public static class ExcludeEmp extends KeyedBroadcastProcessFunction<String, Tuple2<String, String>, String, Tuple2<String, Integer>> {
		...
		public void processBroadcastElement(String empData, Context ctx, Collector<Tuple2<String, Integer>> out)
				throws Exception {
