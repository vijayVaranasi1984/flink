...
public class Bank {
	...
	public static void main(String[] args) throws Exception {
		...
		DataStream<AlarmedCustomer> alarmedCustomers = env.readTextFile("/Flink/bank/alarmed_cust.txt")
		...
		DataStream<Tuple2<String, String>> alarmedCustTransactions = data.keyBy(0).connect(alarmedCustBroadcast)
				.process(new AlarmedCustCheck());

		// (2) Check against lost cards
		DataStream<Tuple2<String, String>> lostCardTransactions = data.keyBy(0).connect(lostCardBroadcast)
		...
		DataStream<Tuple2<String, String>> excessiveTransactions = data
				...
				}).keyBy(0).window(TumblingProcessingTimeWindows.of(Time.seconds(10))).sum(2)
				.flatMap(new FilterAndMapMoreThan10());

		// (4) multiple cities transactions check
...
//				})
		...
		//combining all 4 transactions
		DataStream<Tuple2<String, String>> AllFlaggedTxn = alarmedCustTransactions.union(lostCardTransactions,
				excessiveTransactions, freqCityChangeTransactions);

		AllFlaggedTxn.writeAsText("/Flink/bank/flagged_transaction");
