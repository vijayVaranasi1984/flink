...
public class BankServer {
	...
	public static void main(String[] args) throws Exception {
		...
		DataStream<Tuple2<String, String>> data = env.socketTextStream("localhost", 9094)
		...
		DataStream<AlarmedCustomer> alarmedCustomers = env
				.readTextFile("/Flink/bank/alarmed_cust.txt")
		...
		MapStateDescriptor<String, AlarmedCustomer> alarmedCustStateDescriptor = new MapStateDescriptor<String, AlarmedCustomer>(
		 ...
		 */
		DataStream<LostCard> lostCards = env.readTextFile("/Flink/bank/lost_cards.txt")
				.map(new MapFunction<String, LostCard>() {
					public LostCard map(String value) {
						return new LostCard(value);
					}
				});
		MapStateDescriptor<String, LostCard> lostCardStateDescriptor = new MapStateDescriptor<String, LostCard>(
		...
		/*
		 * (1) Test incoming data against alarmed customers (2) Test incoming data
		 * against lost cards (3) Number of transaction by the customer is more than 10
		 * (4) Transaction city changed frequently
		 */

		DataStream<Tuple2<String, String>> alarmedCustTransactions = data.keyBy(0)
				// (1) Check against alarmed customers
				.connect(alarmedCustBroadcast).process(new AlarmedCustEvaluator());

		DataStream<Tuple2<String, String>> lostCardTransactions = data
				// (2) Check against lost cards
				.keyBy(0).connect(lostCardBroadcast).process(new LostCardEvaluator());
		...
		DataStream<Tuple2<String, String>> excessiveTransactions = data
				...
				}).keyBy(0).timeWindow(Time.minutes(10)).sum(2).flatMap(new FilterAndMapMoreThan10());

		DataStream<Tuple2<String, String>> freqCityChangeTransactions = data
				// (4) Frequenct transaction city changes
				.keyBy(0).flatMap(new TrackCityChanges());
		// .timeWindow(Time.seconds(5));
		// .process(new MyProcessWindowFunction());

		DataStream<Tuple2<String, String>> flaggedTxn = alarmedCustTransactions.union(lostCardTransactions,
				excessiveTransactions, freqCityChangeTransactions);

		flaggedTxn.writeAsText("/Flink/bank/flagged_txn.txt");
	...
	public static class TrackCityChanges extends RichFlatMapFunction<Tuple2<String, String>, Tuple2<String, String>> {
		...
		public void open(Configuration config) {
			ValueStateDescriptor<Tuple2<String, Integer>> descriptor = new ValueStateDescriptor<Tuple2<String, Integer>>(
					"txn_state", TypeInformation.of(new TypeHint<Tuple2<String, Integer>>() {
					}), Tuple2.of("", 0));
	...
	public static class AlarmedCustEvaluator
		...
		private final MapStateDescriptor<String, AlarmedCustomer> alarmedCustStateDescriptor = new MapStateDescriptor<String, AlarmedCustomer>(
				"alarmed_customers", BasicTypeInfo.STRING_TYPE_INFO, Types.POJO(AlarmedCustomer.class));
	...
	public static class LostCardEvaluator
		...
		private final MapStateDescriptor<String, LostCard> lostCardStateDescriptor = new MapStateDescriptor<String, LostCard>(
				"lost_cards", BasicTypeInfo.STRING_TYPE_INFO, Types.POJO(LostCard.class));
